...
#include "fw/multicast-strategy-cache.hpp"
...
typedef StrategyTester<MulticastCacheStrategy> MulticastCacheStrategyTester;
NFD_REGISTER_STRATEGY(MulticastCacheStrategyTester);

class MulticastCacheStrategyFixture : public UnitTestTimeFixture
{
protected:
  MulticastCacheStrategyFixture()
    : strategy(forwarder)
...
protected:
  Forwarder forwarder;
  MulticastCacheStrategyTester strategy;
  Fib& fib;
  Pit& pit;
...
BOOST_FIXTURE_TEST_SUITE(TestMulticastCacheStrategy, MulticastCacheStrategyFixture)

BOOST_AUTO_TEST_CASE(BasicForward)
{
  ...
  const time::nanoseconds TICK = time::duration_cast<time::nanoseconds>(
                                 MulticastCacheStrategy::RETX_SUPPRESSION_INITIAL * 0.1);
  ...
  periodicalRetxFrom4();
  this->advanceClocks(TICK, MulticastCacheStrategy::RETX_SUPPRESSION_MAX * 16);
  scheduler::cancel(retxFrom4Evt);
}

BOOST_AUTO_TEST_CASE(TimeOutForward){
  fib::Entry& fibEntry = *fib.insert(Name()).first;
  fibEntry.addNextHop(*face1, 0);
  fibEntry.addNextHop(*face2, 0);
  fibEntry.addNextHop(*face3, 0);
